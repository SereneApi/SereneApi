...
namespace SereneApi.Extensions.DependencyInjection
{
    public static class SereneApiExtensionsExtensions
    {
        ...
        public static ISereneApiExtensions AddDIAuthenticator<TApi, TDto>([NotNull] this ISereneApiExtensions extensions, [NotNull] Func<TApi, Task<IApiResponse<TDto>>> callApi, [NotNull] Func<TDto, TokenInfo> extractToken) where TApi : class, IDisposable where TDto : class
        {
            if(extensions == null)
            {
